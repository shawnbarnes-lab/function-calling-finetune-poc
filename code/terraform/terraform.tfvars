...
company_name = "shawnderrick-demo"

# Whether the cluster is production or not.
# Set to false for the PoC to avoid requiring iam_merge_request_url
production = false
...
# Or use existing filestore.
# ---
# Shared filesystem for the cluster jail: 1TB per assignment requirement
filestore_jail = {
  spec = {
    size_gibibytes       = 1024
    block_size_kibibytes = 4
...
# ---
# No additional jail submounts for this PoC - the main 1TB shared FS is sufficient
filestore_jail_submounts = []
# Required when filestore_jail_submounts is empty
...
nfs_in_k8s = {
  ...
  size_gibibytes  = 1023  # 1TB-ish for /home (must be multiple of 93 for NETWORK_SSD_IO_M3)
  disk_type       = "NETWORK_SSD_IO_M3"
  filesystem_type = "ext4"
  threads         = 4     # shrunk from 16: nfs-server pod CPU request scales with this,
                          # was requesting 15 CPU which won't fit on a 16vcpu node
}
...
slurm_nodeset_system = {
  min_size = 3   # validation requires at least 3
  max_size = 24
  resource = {
    platform = "cpu-d3"
    preset   = "32vcpu-128gb"  # kept at 32 - system pods need this minimum
  }
  boot_disk = {
    type                 = "NETWORK_SSD"
    size_gibibytes       = 192
...
system_resources = {
  rest = {
    cpu_cores                   = 8     # shrunk: must fit 32vcpu/128gb system nodes alongside other pods
    memory_gibibytes            = 32
  ...
  mariadb = {
    cpu_cores                   = 2     # shrunk: carved out of the 8vcpu/32gb accounting node
    memory_gibibytes            = 12
...
slurm_nodeset_controller = {
  ...
  resource = {
    platform = "cpu-d3"
    preset   = "8vcpu-32gb"   # aggressively shrunk from 64vcpu-256gb for 150 vCPU tenant quota
  }
  boot_disk = {
    type                 = "NETWORK_SSD"
    size_gibibytes       = 128
...
slurm_nodeset_workers = [
  {
    name = "worker"
    # PoC capacity per assignment: 2 nodes of 8x H200 GPUs each = 16 H200 GPUs total
    size = 2
    # Fixed node count for the PoC; disable autoscaling
    autoscaling = {
      enabled = false
      min_size = null
    }
    ...
    gpu_cluster = {
      # InfiniBand fabric per assignment requirement (eu-north2-a)
    ...
    # ---
    # Removed local-data submounts: 2x1TiB network-SSD blew the 6TiB tenant disk quota.
    # Training uses the shared jail filestore; node-local scratch is optional.
    node_local_jail_submounts = []
...
# ---
slurm_nodeset_login = {
  size = 1   # shrunk from 2 to fit 150 vCPU tenant quota
  # Optional. For GB300, this is set to false internally so login pods run on worker nodes.
  # node_group_enabled = true
  resource = {
    platform = "cpu-d3"
    preset   = "16vcpu-64gb"   # 8vcpu was insufficient per Soperator validation; using minimum sufficient
  }
  boot_disk = {
    type                 = "NETWORK_SSD"
    size_gibibytes       = 256   # minimum required by Soperator validation
    block_size_kibibytes = 4
  }
}
...
slurm_nodeset_accounting = {
  resource = {
    platform = "cpu-d3"
    preset   = "8vcpu-32gb"   # aggressively shrunk from 32vcpu-128gb
  }
  boot_disk = {
    type                 = "NETWORK_SSD"
    size_gibibytes       = 128
...
slurm_nodeset_nfs = {
  ...
  resource = {
    platform = "cpu-d3"
    preset   = "16vcpu-64gb"   # shrunk from 128vcpu-512gb to fit 150 vCPU tenant quota
  }
  boot_disk = {
    type                 = "NETWORK_SSD"
    size_gibibytes       = 128
...
slurm_login_ssh_root_public_keys = [
  "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEinWyWZL+mcW0qnswvs7VIusqJ7QV4EPJaNs0L7Z0Gz shawnderrick-nebius-demo",
]
...
# Set to false per assignment guideline (known issue workaround)
...
# ---
backups_password = "<SET_A_STRONG_PASSWORD>"
...
# Published 4.0.1 CRD does not declare plugStackConfig.pyxis.useSquashfuse yet (repo HEAD is ahead of released charts)
enroot_direct_squashfs_enabled = false
